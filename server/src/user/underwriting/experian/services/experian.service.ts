...
export class ExperianService {
  ...
  async creditReportInquiry(
    ...
    try {
      ...
      const requestData = {
        consumerPii: {
          primaryApplicant: {
            ...
            dob: {
              dob: moment(user.dateOfBirth).year(),
            },
            ssn: {
              ssn: '111111111',
            },
        ...
        addOns: {
          ...
          paymentHistory84: 'Y',
          joint: 'N',
          consumerIdentCheck: { getUniqueConsumerIdentifier: '' },
        },
      };
      ...
      const options: AxiosRequestConfig = {
        method: 'POST',
        url: `${exp_url}/consumerservices/credit-profile/v2/credit-report`,
        headers: {
          'Content-Type': 'application/json',
      ...
      const tradeline =
        report.hasOwnProperty('tradeline') && Array.isArray(report.tradeline)
          ...
          : null;
      const fraudShield =
        report.hasOwnProperty('fraudShield') &&
        Array.isArray(report.fraudShield)
          ? report.fraudShield
          : null;
      ...
      const consumerAssistanceReferralAddress = report.hasOwnProperty(
        ...
        : null;
      const directCheck =
        report.hasOwnProperty('directCheck') &&
        Array.isArray(report.directCheck)
          ? report.directCheck
          : null;
      ...
      const create = await this.experianHistoryModel.create({
        createdAt: new Date(),
      ...
      await this.experianHistoryModel.save(create);
      return experianDoc;
    } catch (error) {
      this.logger.error(
        `Error ${error}:`,
